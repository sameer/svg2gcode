...
export function createArtObject(params: {
  ...
  return {
    ...
    aspectLocked: true,
    elementAssignments,
    elementColors: buildElementColorMap(params.preparedSvg.normalized_svg),
  } satisfies ArtObject;
}
...
export function collectAssignmentsForArtObjects(artObjects: ArtObject[]) {
  return artObjects.reduce<Record<string, ElementAssignment>>((result, artObject) => {
    Object.assign(result, artObject.elementAssignments);
    return result;
  }, {});
}
