...
use crate::turtle::collect::Stroke;

fn dist(a: Point<f64>, b: Point<f64>) -> f64 {
    ((a.x - b.x).powi(2) + (a.y - b.y).powi(2)).sqrt()
}

/// Reorder (and optionally reverse) strokes to minimise total pen-up travel distance.
///
/// Uses a greedy nearest-neighbour heuristic for the initial ordering, then refines
/// with tabu search using the Relocate, 2-Opt, and LinkSwap operators.
///
/// <https://www.mdpi.com/2076-3417/9/19/3985/pdf>
pub fn optimize_path_order(strokes: Vec<Stroke>) -> Vec<Stroke> {
    if strokes.len() <= 1 {
        return strokes;
    }
...
/// Greedy nearest-neighbour with per-stroke flip.
///
/// At each step picks the unvisited stroke whose nearest endpoint is closest
/// to the current pen position, reversing it if that gives a shorter approach.
fn nearest_neighbor_greedy(mut remaining: Vec<Stroke>) -> Vec<Stroke> {
    ...
    while !remaining.is_empty() {
        ...
        for (i, stroke) in remaining.iter().enumerate() {
            let d_fwd = dist(pos, stroke.start_point);
            let d_rev = dist(pos, stroke.end_point());
            if d_fwd < best_dist {
                best_dist = d_fwd;
                best_idx = i;
                best_flip = false;
        ...
        let stroke = remaining.swap_remove(best_idx);
        let stroke = if best_flip { stroke.reversed() } else { stroke };
...
enum Operator {
    /// Move a stroke to a different position in the path
    Relocate,
    /// Reverse a sub-sequence of strokes between two edges
    TwoOpt,
...
/// `current_distances[i]` = pen-up distance from stroke i's end to stroke i+1's start.
...
/// Reverse a slice in-place and also call `.reversed()` on each element so that
/// directed strokes connect correctly after the order change.
fn reverse_and_flip(strokes: &mut [Stroke]) {
    strokes.reverse();
    for s in strokes.iter_mut() {
        // Replace in-place without cloning the whole stroke twice.
...
/// - Relocate tries both the normal and flipped orientation of the moved stroke.
...
fn local_improvement_with_tabu_search(path: &[Stroke]) -> Vec<Stroke> {
    ...
    for idx in 0..ITERATIONS {
        if stuck_operators.len() == Operator::NUM_OPERATORS {
            if tabu.is_empty() {
                info!("TSP: stuck after {idx} iterations, no more local improvements");
                break;
            } else {
            ...
            Operator::Relocate => {
                let best_move = (1..current.len().saturating_sub(1))
                    ...
                    .map(|(i, j, unlink_improvement)| {
                        ...
                        let neg_flipped = dist(current[j].end_point(), current[i].end_point())
                            + dist(current[i].start_point, current[j + 1].start_point);

                        if neg_normal <= neg_flipped {
                            (i, j, false, positive_diff - neg_normal)
                        } else {
                            (i, j, true, positive_diff - neg_flipped)
                        }
                    })
                    .max_by(|a, b| a.3.partial_cmp(&b.3).unwrap_or(std::cmp::Ordering::Equal));

                if let Some((i, j, flipped, diff)) = best_move {
                    if diff <= 0.0 {
                    ...
                    .map(|(this, other)| {
                        ...
                        let diff = (current_distances[this.0] + current_distances[other.0])
                            - (dist(current[this.0].end_point(), current[other.0].end_point())
                                + dist(current[this.1].start_point, current[other.1].start_point));
                        (this, other, diff)
                    })
            ...
            Operator::LinkSwap => {
                ...
                let best_move = (2..current.len().saturating_sub(1))
                    ...
                    .map(|(i, j)| {
                        ...
                        //   0 = [from, last_end]: suffix [j..] reversed+flipped
                        //   1 = [first_start, to]: prefix [..=i] reversed+flipped
